...
function forum_cron() {
    ...
    $site = get_site();
    $output = $PAGE->get_renderer('mod_forum', 'email');
    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            foreach ($posts as $pid => $post) {
                ...
                $forumpostmailtext = new \mod_forum\output\email\forum_post_mail_text($course, $cm, $forum,
                        $discussion, $post, $userfrom, $userto, false, $replyaddress);
                $posttext = $output->render($forumpostmailtext);

                $forumpostmailhtml = new \mod_forum\output\email\forum_post_mail_html($course, $cm, $forum,
                        $discussion, $post, $userfrom, $userto, $replyaddress);
                $posthtml = $output->render($forumpostmailhtml);
                ...
                foreach ($thesediscussions as $discussionid) {
                    ...
                    foreach ($postsarray as $postid) {
                        ...
                        if ($maildigest == 2) {
                            ...
                            // The full treatment
                            $forumpostmailtext = new mod_forum\output\email\forum_post_mail_text($course, $cm, $forum, $discussion,
                                    $post, $userfrom, $userto, false);
                            $posttext .= $output->render($forumpostmailtext);

                            $forumposthtml = new mod_forum\output\email\forum_post_html($course, $cm, $forum, $discussion,
                                    $post, $userfrom, $userto, false, $canreply, true, false);
                            $posthtml .= $output->render($forumposthtml);
